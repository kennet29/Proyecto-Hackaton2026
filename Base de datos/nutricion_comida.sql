...
if object_id('dbo.nutricioncomida', 'U') is null
begin
    create table dbo.nutricioncomida (
...
if not exists (select 1 from sys.indexes where name = 'ix_nutricioncomida_paciente_fecha' and object_id = object_id('dbo.nutricioncomida'))
begin
    create index ix_nutricioncomida_paciente_fecha
        on dbo.nutricioncomida(pacienteid, fechaanalisis desc);
end;
go

if not exists (select 1 from sys.indexes where name = 'ix_nutricioncomida_consulta' and object_id = object_id('dbo.nutricioncomida'))
begin
    create index ix_nutricioncomida_consulta
        on dbo.nutricioncomida(consultaid)
        where consultaid is not null;
end;
go

if not exists (select 1 from sys.indexes where name = 'ux_nutricioncomida_paciente_consulta_fecha' and object_id = object_id('dbo.nutricioncomida'))
begin
    create unique index ux_nutricioncomida_paciente_consulta_fecha
        on dbo.nutricioncomida(pacienteid, consultaid, fechaanalisis, origen);
end;
go
