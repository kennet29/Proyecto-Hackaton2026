...
/**
 * Implementa la lógica de negocio y persistencia del dominio mail.
 */
@Injectable()
export class MailService {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    if (host && user && pass) {
      ...
      this.logger.warn(
        "correo smtp no configurado (variables MAIL_* faltantes)",
      );
    }
  }
   ...
   * @param token Valor del parámetro `token`.
  ...
  async sendPasswordResetMail(to: string, token: string, expiresAt: Date) {
    if (!this.transporter || !this.fromAddress) {
      this.logger.warn(
        `no se envio correo a ${to}: transporter o remitente no configurados`,
      );
      return;
    }
