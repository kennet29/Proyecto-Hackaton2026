...
const PRIMARY_KEYS = ["estilovidaId"];
...
/**
 * Implementa la lógica de negocio y persistencia del dominio estilovida.
 */
@Injectable()
export class EstilovidaService {
   ...
   */
  create(payload: CreateEstilovidaDto): Promise<Estilovida> {
    const entity = this.estilovidaRepository.create(
      payload as Partial<Estilovida>,
    );
    return this.estilovidaRepository.save(entity);
  }
   ...
   */
  findAll(): Promise<Estilovida[]> {
    return this.estilovidaRepository.find();
  }
   ...
   */
  async findOne(id: string): Promise<Estilovida> {
    const where = this.parseId(id);
    const entity = await this.estilovidaRepository.findOne({ where });
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en estilovida`);
    }
    return entity;
  }
   ...
   */
  async update(id: string, payload: UpdateEstilovidaDto): Promise<Estilovida> {
  ...
  async remove(id: string): Promise<void> {
    const where = this.parseId(id);
    const result = await this.estilovidaRepository.delete(where);
    if (!result.affected) {
      throw new NotFoundException(`registro ${id} no encontrado en estilovida`);
    }
  }
