...
const PRIMARY_KEYS = ["tipolesionId"];
...
/**
 * Implementa la lógica de negocio y persistencia del dominio tipolesion.
 */
@Injectable()
export class TipolesionService {
  constructor(
    @InjectRepository(Tipolesion)
    private readonly tipolesionRepository: Repository<Tipolesion>,
  ) {}
   ...
   */
  create(payload: CreateTipolesionDto): Promise<Tipolesion> {
    const entity = this.tipolesionRepository.create(
      payload as Partial<Tipolesion>,
    );
    return this.tipolesionRepository.save(entity);
  }
   ...
   */
  findAll(): Promise<Tipolesion[]> {
    return this.tipolesionRepository.find();
  }
   ...
   */
  async findOne(id: string): Promise<Tipolesion> {
    const where = this.parseId(id);
    const entity = await this.tipolesionRepository.findOne({ where });
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en tipolesion`);
    }
    return entity;
  }
   ...
   */
  async update(id: string, payload: UpdateTipolesionDto): Promise<Tipolesion> {
  ...
  async remove(id: string): Promise<void> {
    const where = this.parseId(id);
    const result = await this.tipolesionRepository.delete(where);
    if (!result.affected) {
      throw new NotFoundException(`registro ${id} no encontrado en tipolesion`);
    }
  }
