...
/**
 * Implementa la lógica de negocio y persistencia del dominio puntajeriesgo.
 */
@Injectable()
export class PuntajeriesgoService {
  ...
  create(payload: CreatePuntajeriesgoDto): Promise<Puntajeriesgo> {
    const entity = this.puntajeriesgoRepository.create(
      payload as Partial<Puntajeriesgo>,
    );
  ...
  async findOne(id: string): Promise<Puntajeriesgo> {
    ...
    if (!entity) {
      throw new NotFoundException(
        `registro ${id} no encontrado en puntajeriesgo`,
      );
    }
    return entity;
  }
  ...
  async update(
    id: string,
    payload: UpdatePuntajeriesgoDto,
  ): Promise<Puntajeriesgo> {
