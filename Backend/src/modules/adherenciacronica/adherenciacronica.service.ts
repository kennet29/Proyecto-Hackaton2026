...
/**
 * Implementa la lógica de negocio y persistencia del dominio adherenciacronica.
 */
@Injectable()
export class AdherenciacronicaService {
  ...
  create(payload: CreateAdherenciacronicaDto): Promise<Adherenciacronica> {
    const entity = this.adherenciacronicaRepository.create(
      payload as Partial<Adherenciacronica>,
    );
  ...
  async findOne(id: string): Promise<Adherenciacronica> {
    ...
    if (!entity) {
      throw new NotFoundException(
        `registro ${id} no encontrado en adherenciacronica`,
      );
    }
    return entity;
  }
  ...
  async update(
    id: string,
    payload: UpdateAdherenciacronicaDto,
  ): Promise<Adherenciacronica> {
