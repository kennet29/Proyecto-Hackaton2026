...
const PRIMARY_KEYS = ["controlcronicoId"];
...
/**
 * Implementa la lógica de negocio y persistencia del dominio controlcronico.
 */
@Injectable()
export class ControlcronicoService {
  constructor(
    @InjectRepository(Controlcronico)
    private readonly controlcronicoRepository: Repository<Controlcronico>,
  ) {}
   ...
   */
  create(payload: CreateControlcronicoDto): Promise<Controlcronico> {
    const entity = this.controlcronicoRepository.create(
      payload as Partial<Controlcronico>,
    );
    return this.controlcronicoRepository.save(entity);
  }
   ...
   */
  findAll(): Promise<Controlcronico[]> {
    return this.controlcronicoRepository.find();
  }
   ...
   */
  async findOne(id: string): Promise<Controlcronico> {
    const where = this.parseId(id);
    const entity = await this.controlcronicoRepository.findOne({ where });
    if (!entity) {
      throw new NotFoundException(
        `registro ${id} no encontrado en controlcronico`,
      );
    }
    return entity;
  }
  ...
  async update(
    id: string,
    payload: UpdateControlcronicoDto,
  ): Promise<Controlcronico> {
  ...
  async remove(id: string): Promise<void> {
    const where = this.parseId(id);
    const result = await this.controlcronicoRepository.delete(where);
    if (!result.affected) {
      throw new NotFoundException(
        `registro ${id} no encontrado en controlcronico`,
      );
