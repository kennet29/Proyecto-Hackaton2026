...
export class ControlcronicoService {
  ...
  create(payload: CreateControlcronicoDto): Promise<Controlcronico> {
    const entity = this.controlcronicoRepository.create(payload as Partial<Controlcronico>);
  ...
  async findOne(id: string): Promise<Controlcronico> {
    ...
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en controlcronico`);
    }
    return entity;
  }

  async update(id: string, payload: UpdateControlcronicoDto): Promise<Controlcronico> {
