...
/**
 * Implementa la lógica de negocio y persistencia del dominio citamedica.
 */
@Injectable()
export class CitamedicaService {
   ...
   */
  create(payload: CreateCitamedicaDto): Promise<Citamedica> {
    const entity = this.citamedicaRepository.create(
      payload as Partial<Citamedica>,
    );
    return this.citamedicaRepository.save(entity);
  }
  ...
  async findOne(id: string): Promise<Citamedica> {
    ...
    const entity = await this.citamedicaRepository.findOne({ where });
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en citamedica`);
    }
    return entity;
  }
   ...
   */
  async update(id: string, payload: UpdateCitamedicaDto): Promise<Citamedica> {
  ...
  async remove(id: string): Promise<void> {
    const where = this.parseId(id);
    const result = await this.citamedicaRepository.delete(where);
    if (!result.affected) {
      throw new NotFoundException(`registro ${id} no encontrado en citamedica`);
    }
  }
