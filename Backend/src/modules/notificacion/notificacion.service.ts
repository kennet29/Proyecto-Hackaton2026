...
export class NotificacionService {
  ...
  create(payload: CreateNotificacionDto): Promise<Notificacion> {
    const entity = this.notificacionRepository.create(payload as Partial<Notificacion>);
  ...
  async findOne(id: string): Promise<Notificacion> {
    ...
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en notificacion`);
    }
    return entity;
  }

  async update(id: string, payload: UpdateNotificacionDto): Promise<Notificacion> {
