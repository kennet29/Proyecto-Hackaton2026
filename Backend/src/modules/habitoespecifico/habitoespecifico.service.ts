...
/**
 * Implementa la lógica de negocio y persistencia del dominio habitoespecifico.
 */
@Injectable()
export class HabitoespecificoService {
  constructor(
    @InjectRepository(Habitoespecifico)
    private readonly habitoespecificoRepository: Repository<Habitoespecifico>,
  ) {}
   ...
   */
  create(payload: CreateHabitoespecificoDto): Promise<Habitoespecifico> {
    const entity = this.habitoespecificoRepository.create(
      payload as Partial<Habitoespecifico>,
    );
    return this.habitoespecificoRepository.save(entity);
  }
   ...
   */
  findAll(): Promise<Habitoespecifico[]> {
    return this.habitoespecificoRepository.find();
  }
   ...
   */
  async findOne(id: string): Promise<Habitoespecifico> {
    const where = this.parseId(id);
    const entity = await this.habitoespecificoRepository.findOne({ where });
    if (!entity) {
      throw new NotFoundException(
        `registro ${id} no encontrado en habitoespecifico`,
      );
    }
    return entity;
  }
  ...
  async update(
    id: string,
    payload: UpdateHabitoespecificoDto,
  ): Promise<Habitoespecifico> {
  ...
  async remove(id: string): Promise<void> {
    const where = this.parseId(id);
    const result = await this.habitoespecificoRepository.delete(where);
    if (!result.affected) {
      throw new NotFoundException(
        `registro ${id} no encontrado en habitoespecifico`,
      );
