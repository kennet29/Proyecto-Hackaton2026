...
export class HabitoespecificoService {
  ...
  create(payload: CreateHabitoespecificoDto): Promise<Habitoespecifico> {
    const entity = this.habitoespecificoRepository.create(payload as Partial<Habitoespecifico>);
  ...
  async findOne(id: string): Promise<Habitoespecifico> {
    ...
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en habitoespecifico`);
    }
    return entity;
  }

  async update(id: string, payload: UpdateHabitoespecificoDto): Promise<Habitoespecifico> {
