...
/**
 * Implementa la lógica de negocio y persistencia del dominio evaluacionsaludhabito.
 */
@Injectable()
export class EvaluacionsaludhabitoService {
   ...
   */
  create(
    payload: CreateEvaluacionsaludhabitoDto,
  ): Promise<Evaluacionsaludhabito> {
    const entity = this.evaluacionsaludhabitoRepository.create(
      payload as Partial<Evaluacionsaludhabito>,
    );
  ...
  async findOne(id: string): Promise<Evaluacionsaludhabito> {
    ...
    const entity = await this.evaluacionsaludhabitoRepository.findOne({
      where,
    });
    if (!entity) {
      throw new NotFoundException(
        `registro ${id} no encontrado en evaluacionsaludhabito`,
      );
    }
    return entity;
  }
  ...
  async update(
    id: string,
    payload: UpdateEvaluacionsaludhabitoDto,
  ): Promise<Evaluacionsaludhabito> {
