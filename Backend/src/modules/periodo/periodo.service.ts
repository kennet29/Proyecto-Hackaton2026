...
export class PeriodoService {
  ...
  async create(payload: CreatePeriodoDto): Promise<PeriodoResponse> {
    ...
    const entity = this.periodoRepository.create(this.mapPayloadForSave(payload, true));
  ...
  async update(id: number, payload: UpdatePeriodoDto): Promise<PeriodoResponse> {
    const entity = await this.findEntity(id);
  ...
  async getPrediction(pacienteId: number) {
    ...
    return {
      ...
      ventanaFertil: {
        inicio: this.toIsoDate(this.addDays(new Date(prediction.fechaInicio), -19)),
        fin: this.toIsoDate(this.addDays(new Date(prediction.fechaInicio), -13)),
      },
  ...
  async getCalendar(pacienteId: number, month?: number, year?: number) {
    ...
    const registros = records
      .filter((record) =>
        this.overlapsMonth(record.fechaInicio, record.fechaFin, monthStart, monthEnd),
      )
      .map((record) => ({
  ...
  async getMedicalReport(pacienteId: number) {
    ...
    if (variance >= 8) {
      alertas.push('Los ciclos registrados muestran variaciones amplias entre periodos.');
    }
    if ((historial.promedioDuracionDias ?? 0) >= 8) {
      alertas.push('La duracion promedio registrada es mayor o igual a 8 dias.');
    }
    if (!alertas.length) {
      alertas.push('No se detectaron patrones llamativos con los datos disponibles.');
    }
    ...
    return {
      ...
      alertas,
      nota:
        'Este reporte es un apoyo para seguimiento y no reemplaza la valoracion de un profesional de salud.',
    };
  }
  ...
  private buildPrediction(records: Periodo[]): PredictionEvent {
    ...
    const durations = records
      .map((record) => record.duracionDias)
      .filter((value): value is number => typeof value === 'number' && value > 0);
    const fallbackCycle = lastRecord.cicloDias ?? this.average(cycleLengths) ?? 28;
    const fallbackDuration = this.average(durations) ?? lastRecord.duracionDias ?? 5;
  ...
  private buildPredictionSequence(
    ...
    for (let index = 1; index < count; index += 1) {
      ...
      const nextStart = this.addDays(new Date(previous.fechaInicio), previous.cicloDias);
  ...
  private calculateCycleLengths(records: Periodo[]): number[] {
    ...
    for (let index = 1; index < records.length; index += 1) {
      const diff = this.diffDays(records[index - 1].fechaInicio, records[index].fechaInicio);
      if (diff > 0) {
        values.push(diff);
      }
    }
