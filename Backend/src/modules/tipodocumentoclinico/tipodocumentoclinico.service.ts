...
/**
 * Implementa la lógica de negocio y persistencia del dominio tipodocumentoclinico.
 */
@Injectable()
export class TipodocumentoclinicoService {
   ...
   */
  create(
    payload: CreateTipodocumentoclinicoDto,
  ): Promise<Tipodocumentoclinico> {
    const entity = this.tipodocumentoclinicoRepository.create(
      payload as Partial<Tipodocumentoclinico>,
    );
  ...
  async findOne(id: string): Promise<Tipodocumentoclinico> {
    ...
    if (!entity) {
      throw new NotFoundException(
        `registro ${id} no encontrado en tipodocumentoclinico`,
      );
    }
    return entity;
  }
  ...
  async update(
    id: string,
    payload: UpdateTipodocumentoclinicoDto,
  ): Promise<Tipodocumentoclinico> {
