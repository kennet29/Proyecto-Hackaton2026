...
/**
 * Implementa la lógica de negocio y persistencia del dominio tipooperacion.
 */
@Injectable()
export class TipooperacionService {
  ...
  create(payload: CreateTipooperacionDto): Promise<Tipooperacion> {
    const entity = this.tipooperacionRepository.create(
      payload as Partial<Tipooperacion>,
    );
  ...
  async findOne(id: string): Promise<Tipooperacion> {
    ...
    if (!entity) {
      throw new NotFoundException(
        `registro ${id} no encontrado en tipooperacion`,
      );
    }
    return entity;
  }
  ...
  async update(
    id: string,
    payload: UpdateTipooperacionDto,
