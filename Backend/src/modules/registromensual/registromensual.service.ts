...
const PRIMARY_KEYS = ["registromensualId"];
...
export class RegistromensualService {
  ...
  create(payload: CreateRegistromensualDto): Promise<Registromensual> {
    const entity = this.registromensualRepository.create(payload as Partial<Registromensual>);
    return this.registromensualRepository.save(entity);
  }

  findAll(): Promise<Registromensual[]> {
    return this.registromensualRepository.find();
  }

  async findOne(id: string): Promise<Registromensual> {
    const where = this.parseId(id);
    const entity = await this.registromensualRepository.findOne({ where });
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en registromensual`);
    }
    return entity;
  }

  async update(id: string, payload: UpdateRegistromensualDto): Promise<Registromensual> {
  ...
  async remove(id: string): Promise<void> {
    const where = this.parseId(id);
    const result = await this.registromensualRepository.delete(where);
    if (!result.affected) {
      throw new NotFoundException(`registro ${id} no encontrado en registromensual`);
    }
  }
