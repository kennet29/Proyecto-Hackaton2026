...
/**
 * Implementa la lógica de negocio y persistencia del dominio registromensual.
 */
@Injectable()
export class RegistromensualService {
  ...
  create(payload: CreateRegistromensualDto): Promise<Registromensual> {
    const entity = this.registromensualRepository.create(
      payload as Partial<Registromensual>,
    );
  ...
  async findOne(id: string): Promise<Registromensual> {
    ...
    if (!entity) {
      throw new NotFoundException(
        `registro ${id} no encontrado en registromensual`,
      );
    }
    return entity;
  }
  ...
  async update(
    id: string,
    payload: UpdateRegistromensualDto,
