...
const PRIMARY_KEYS = ["rolId", "permisoId"];
...
/**
 * Implementa la lógica de negocio y persistencia del dominio rolpermiso.
 */
@Injectable()
export class RolpermisoService {
   ...
   */
  create(payload: CreateRolpermisoDto): Promise<Rolpermiso> {
    const entity = this.rolpermisoRepository.create(
      payload as Partial<Rolpermiso>,
    );
    return this.rolpermisoRepository.save(entity);
  }
  ...
  async findOne(id: string): Promise<Rolpermiso> {
    ...
    const entity = await this.rolpermisoRepository.findOne({ where });
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en rolpermiso`);
    }
    return entity;
  }
   ...
   */
  async update(id: string, payload: UpdateRolpermisoDto): Promise<Rolpermiso> {
  ...
  async remove(id: string): Promise<void> {
    const where = this.parseId(id);
    const result = await this.rolpermisoRepository.delete(where);
    if (!result.affected) {
      throw new NotFoundException(`registro ${id} no encontrado en rolpermiso`);
    }
  }
