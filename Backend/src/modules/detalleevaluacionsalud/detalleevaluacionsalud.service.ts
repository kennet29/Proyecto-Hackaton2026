...
/**
 * Implementa la lógica de negocio y persistencia del dominio detalleevaluacionsalud.
 */
@Injectable()
export class DetalleevaluacionsaludService {
   ...
   */
  create(
    payload: CreateDetalleevaluacionsaludDto,
  ): Promise<Detalleevaluacionsalud> {
    const entity = this.detalleevaluacionsaludRepository.create(
      payload as Partial<Detalleevaluacionsalud>,
    );
  ...
  async findOne(id: string): Promise<Detalleevaluacionsalud> {
    ...
    const entity = await this.detalleevaluacionsaludRepository.findOne({
      where,
    });
    if (!entity) {
      throw new NotFoundException(
        `registro ${id} no encontrado en detalleevaluacionsalud`,
      );
    }
    return entity;
  }
  ...
  async update(
    id: string,
    payload: UpdateDetalleevaluacionsaludDto,
  ): Promise<Detalleevaluacionsalud> {
