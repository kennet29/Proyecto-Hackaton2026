...
const PRIMARY_KEYS = ["detalleId"];
const PRIMARY_KEY_TYPES: Record<string, 'number' | 'string' | 'boolean' | 'Date'> = {
  detalleId: 'number',
};

@Injectable()
export class DetalleevaluacionsaludService {
  constructor(
    @InjectRepository(Detalleevaluacionsalud)
    private readonly detalleevaluacionsaludRepository: Repository<Detalleevaluacionsalud>,
  ) {}

  create(payload: CreateDetalleevaluacionsaludDto): Promise<Detalleevaluacionsalud> {
    const entity = this.detalleevaluacionsaludRepository.create(payload as Partial<Detalleevaluacionsalud>);
    return this.detalleevaluacionsaludRepository.save(entity);
  }

  findAll(): Promise<Detalleevaluacionsalud[]> {
    return this.detalleevaluacionsaludRepository.find();
  }

  async findOne(id: string): Promise<Detalleevaluacionsalud> {
    const where = this.parseId(id);
    const entity = await this.detalleevaluacionsaludRepository.findOne({ where });
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en detalleevaluacionsalud`);
    }
    return entity;
  }

  async update(id: string, payload: UpdateDetalleevaluacionsaludDto): Promise<Detalleevaluacionsalud> {
  ...
  async remove(id: string): Promise<void> {
    const where = this.parseId(id);
    const result = await this.detalleevaluacionsaludRepository.delete(where);
    if (!result.affected) {
      throw new NotFoundException(`registro ${id} no encontrado en detalleevaluacionsalud`);
    }
  }
