...
const PRIMARY_KEYS = ["operacionId"];
...
/**
 * Implementa la lógica de negocio y persistencia del dominio operacion.
 */
@Injectable()
export class OperacionService {
   ...
   */
  create(payload: CreateOperacionDto): Promise<Operacion> {
    const entity = this.operacionRepository.create(
      payload as Partial<Operacion>,
    );
    return this.operacionRepository.save(entity);
  }
  ...
  async findOne(id: string): Promise<Operacion> {
    ...
    const entity = await this.operacionRepository.findOne({ where });
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en operacion`);
    }
    return entity;
  }
   ...
   */
  async update(id: string, payload: UpdateOperacionDto): Promise<Operacion> {
  ...
  async remove(id: string): Promise<void> {
    const where = this.parseId(id);
    const result = await this.operacionRepository.delete(where);
    if (!result.affected) {
      throw new NotFoundException(`registro ${id} no encontrado en operacion`);
    }
  }
