...
/**
 * Implementa la lógica de negocio y persistencia del dominio alergia.
 */
@Injectable()
export class AlergiaService {
   ...
   */
  create(payload: CreateAlergiaDto): Promise<Alergia> {
    const entity = this.alergiaRepository.create(payload as Partial<Alergia>);
    return this.alergiaRepository.save(entity);
  }
  ...
  async findOne(id: string): Promise<Alergia> {
    ...
    const entity = await this.alergiaRepository.findOne({ where });
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en alergia`);
    }
    return entity;
  }
   ...
   */
  async update(id: string, payload: UpdateAlergiaDto): Promise<Alergia> {
  ...
  async remove(id: string): Promise<void> {
    const where = this.parseId(id);
    const result = await this.alergiaRepository.delete(where);
    if (!result.affected) {
      throw new NotFoundException(`registro ${id} no encontrado en alergia`);
    }
  }
