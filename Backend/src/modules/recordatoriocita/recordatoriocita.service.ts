...
const PRIMARY_KEYS = ["recordatoriocitaId"];
...
export class RecordatoriocitaService {
  ...
  create(payload: CreateRecordatoriocitaDto): Promise<Recordatoriocita> {
    const entity = this.recordatoriocitaRepository.create(payload as Partial<Recordatoriocita>);
    return this.recordatoriocitaRepository.save(entity);
  }

  findAll(): Promise<Recordatoriocita[]> {
    return this.recordatoriocitaRepository.find();
  }

  async findOne(id: string): Promise<Recordatoriocita> {
    const where = this.parseId(id);
    const entity = await this.recordatoriocitaRepository.findOne({ where });
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en recordatoriocita`);
    }
    return entity;
  }

  async update(id: string, payload: UpdateRecordatoriocitaDto): Promise<Recordatoriocita> {
  ...
  async remove(id: string): Promise<void> {
    const where = this.parseId(id);
    const result = await this.recordatoriocitaRepository.delete(where);
    if (!result.affected) {
      throw new NotFoundException(`registro ${id} no encontrado en recordatoriocita`);
    }
  }
