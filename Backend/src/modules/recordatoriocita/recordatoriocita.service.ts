...
/**
 * Implementa la lógica de negocio y persistencia del dominio recordatoriocita.
 */
@Injectable()
export class RecordatoriocitaService {
  ...
  create(payload: CreateRecordatoriocitaDto): Promise<Recordatoriocita> {
    const entity = this.recordatoriocitaRepository.create(
      payload as Partial<Recordatoriocita>,
    );
  ...
  async findOne(id: string): Promise<Recordatoriocita> {
    ...
    if (!entity) {
      throw new NotFoundException(
        `registro ${id} no encontrado en recordatoriocita`,
      );
    }
    return entity;
  }
  ...
  async update(
    id: string,
    payload: UpdateRecordatoriocitaDto,
