...
/**
 * Implementa la lógica de negocio y persistencia del dominio registrodental.
 */
@Injectable()
export class RegistrodentalService {
  ...
  create(payload: CreateRegistrodentalDto): Promise<Registrodental> {
    const entity = this.registrodentalRepository.create(
      payload as Partial<Registrodental>,
    );
  ...
  async findOne(id: string): Promise<Registrodental> {
    ...
    if (!entity) {
      throw new NotFoundException(
        `registro ${id} no encontrado en registrodental`,
      );
    }
    return entity;
  }
  ...
  async update(
    id: string,
    payload: UpdateRegistrodentalDto,
  ): Promise<Registrodental> {
