...
export class AntecedentefamiliarService {
  ...
  create(payload: CreateAntecedentefamiliarDto): Promise<Antecedentefamiliar> {
    const entity = this.antecedentefamiliarRepository.create(payload as Partial<Antecedentefamiliar>);
  ...
  async findOne(id: string): Promise<Antecedentefamiliar> {
    ...
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en antecedentefamiliar`);
    }
    return entity;
  }

  async update(id: string, payload: UpdateAntecedentefamiliarDto): Promise<Antecedentefamiliar> {
