...
export class ConsultamedicaService {
  ...
  create(payload: CreateConsultamedicaDto): Promise<Consultamedica> {
    const entity = this.consultamedicaRepository.create(payload as Partial<Consultamedica>);
  ...
  async findOne(id: string): Promise<Consultamedica> {
    ...
    if (!entity) {
      throw new NotFoundException(`registro ${id} no encontrado en consultamedica`);
    }
    return entity;
  }

  async update(id: string, payload: UpdateConsultamedicaDto): Promise<Consultamedica> {
