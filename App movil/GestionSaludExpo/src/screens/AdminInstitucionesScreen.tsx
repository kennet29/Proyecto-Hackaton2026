/**
 * @file App movil/GestionSaludExpo/src/screens/AdminInstitucionesScreen.tsx
...
const createEnhancedStyles = (colors: AppColors) => StyleSheet.create({
  overview: { flexDirection: 'row', gap: 8 },
  stat: { flex: 1, minHeight: 64, flexDirection: 'row', alignItems: 'center', gap: 8, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface },
  statIcon: { height: 29, width: 29, alignItems: 'center', justifyContent: 'center', borderRadius: 9 },
  statValue: { color: colors.text, fontSize: 18, fontWeight: '900' },
  statLabel: { color: colors.textMuted, fontSize: 10, lineHeight: 12 },
  searchBox: { minHeight: 48, flexDirection: 'row', alignItems: 'center', gap: 9, borderRadius: 14, borderWidth: 1, borderColor: colors.borderStrong, paddingHorizontal: 13, backgroundColor: colors.surfaceStrong },
  ...
  serviceIndicatorText: { color: colors.textMuted, fontSize: 12, fontWeight: '700' },
...
export function AdminInstitucionesScreen({ navigation }: Props) {
  ...
  const enhancedStyles = React.useMemo(() => createEnhancedStyles(colors), [colors]);
  ...
  return <SafeAreaView style={styles.safe} edges={['bottom']}><ScrollView contentContainerStyle={styles.container}>
    ...
    <View style={enhancedStyles.overview}>
      <Stat icon="business-outline" value={clinics.length} label="Instituciones" color={colors.info} />
      <Stat icon="checkmark-circle-outline" value={activeClinicCount} label="Activas" color={colors.success} />
    ...
    <View style={enhancedStyles.searchBox}>
      ...
      <AppTextInput value={query} onChangeText={setQuery} placeholder={tab === 'clinicas' ? 'Buscar por nombre, ciudad o tipo' : 'Buscar por servicio o categoría'} placeholderTextColor={colors.textMuted} style={enhancedStyles.searchInput} />
    ...
    {loading ? <ActivityIndicator color={colors.info} size="large" style={styles.loader}/> : tab === 'clinicas' ? <>
...
function TabButton({ active, icon, label, onPress }: { active: boolean; icon: keyof typeof Ionicons.glyphMap; label: string; onPress: () => void }) {
  const colors = useAppColors();
  const styles = React.useMemo(() => createStyles(colors), [colors]);
 return <TouchableOpacity onPress={onPress} style={[styles.tab, active && styles.tabActive]}><Ionicons name={icon} size={18} color={active ? colors.background : colors.textMuted}/><AppText style={[styles.tabText, active && styles.tabTextActive]}>{label}</AppText></TouchableOpacity>; }
function Stat({ icon, value, label, color }: { icon: keyof typeof Ionicons.glyphMap; value: number; label: string; color: string }) {
  const colors = useAppColors();
  const enhancedStyles = React.useMemo(() => createEnhancedStyles(colors), [colors]);
 return <View style={enhancedStyles.stat}><View style={[enhancedStyles.statIcon, { backgroundColor: `${color}1F` }]}><Ionicons name={icon} size={16} color={color}/></View><View><AppText style={enhancedStyles.statValue}>{value}</AppText><AppText style={enhancedStyles.statLabel}>{label}</AppText></View></View>; }
function Tag({ label }: { label: string }) {
  const colors = useAppColors();
  const enhancedStyles = React.useMemo(() => createEnhancedStyles(colors), [colors]);
 return <View style={enhancedStyles.tag}><AppText style={enhancedStyles.tagText}>{label}</AppText></View>; }
function Action({ label, icon, onPress }: { label: string; icon: keyof typeof Ionicons.glyphMap; onPress: () => void }) {
  const colors = useAppColors();
  const styles = React.useMemo(() => createStyles(colors), [colors]);
 return <TouchableOpacity style={styles.primary} onPress={onPress}><Ionicons name={icon} size={19} color={colors.onAccent}/><AppText style={styles.primaryText}>{label}</AppText></TouchableOpacity>; }
function SmallAction({ label, icon, onPress }: { label: string; icon: keyof typeof Ionicons.glyphMap; onPress: () => void }) {
  const colors = useAppColors();
  const styles = React.useMemo(() => createStyles(colors), [colors]);
 return <TouchableOpacity style={styles.smallAction} onPress={onPress}><Ionicons name={icon} size={16} color={colors.info}/><AppText style={styles.smallText}>{label}</AppText></TouchableOpacity>; }
function Status({ active }: { active: boolean }) {
  const colors = useAppColors();
  const styles = React.useMemo(() => createStyles(colors), [colors]);
 return <View style={[styles.status, { backgroundColor: active ? `${colors.success}24` : `${colors.accent}24` }]}><AppText style={{ color: active ? colors.success : colors.accent, fontSize: 11, fontWeight: '800' }}>{active ? 'ACTIVA' : 'INACTIVA'}</AppText></View>; }
function Empty({ icon, text }: { icon: keyof typeof Ionicons.glyphMap; text: string }) {
  const colors = useAppColors();
  const styles = React.useMemo(() => createStyles(colors), [colors]);
 return <View style={styles.empty}><Ionicons name={icon} size={34} color={colors.textMuted}/><AppText style={styles.helper}>{text}</AppText></View>; }

function Field({ label, value, onChangeText, keyboardType = 'default', multiline = false }: { label: string; value: string; onChangeText: (value: string) => void; keyboardType?: 'default' | 'email-address' | 'phone-pad'; multiline?: boolean }) {
  const colors = useAppColors();
  const styles = React.useMemo(() => createStyles(colors), [colors]);
 return <View style={styles.field}><AppText style={styles.fieldLabel}>{label}</AppText><AppTextInput style={[styles.input, multiline && styles.textarea]} value={value} onChangeText={onChangeText} keyboardType={keyboardType} autoCapitalize="sentences" multiline={multiline} /></View>; }
function BooleanRow({ label, value, onChange }: { label: string; value: boolean; onChange: (value: boolean) => void }) {
  const colors = useAppColors();
  const styles = React.useMemo(() => createStyles(colors), [colors]);
 return <TouchableOpacity style={styles.booleanRow} onPress={() => onChange(!value)}><View style={[styles.checkbox, value && styles.checkboxActive]}>{value ? <Ionicons name="checkmark" size={15} color={colors.onAccent}/> : null}</View><AppText style={styles.booleanText}>{label}</AppText></TouchableOpacity>; }
function FormModal({ visible, title, children, saving, onClose, onSave }: { visible: boolean; title: string; children: React.ReactNode; saving: boolean; onClose: () => void; onSave: () => void }) {
  const colors = useAppColors();
  const styles = React.useMemo(() => createStyles(colors), [colors]);
 return <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}><View style={styles.overlay}><View style={styles.modal}><View style={styles.modalHeader}><AppText style={styles.modalTitle}>{title}</AppText><TouchableOpacity onPress={onClose}><Ionicons name="close" size={24} color={colors.text}/></TouchableOpacity></View><ScrollView showsVerticalScrollIndicator={false}>{children}<TouchableOpacity style={[styles.primary, saving && styles.disabled]} disabled={saving} onPress={onSave}>{saving ? <ActivityIndicator color={colors.onAccent}/> : <><Ionicons name="save-outline" size={19} color={colors.onAccent}/><AppText style={styles.primaryText}>Guardar cambios</AppText></>}</TouchableOpacity></ScrollView></View></View></Modal>; }
function ClinicModal({ visible, form, setForm, saving, editing, onClose, onSave }: { visible: boolean; form: ReturnType<typeof emptyClinic>; setForm: React.Dispatch<React.SetStateAction<ReturnType<typeof emptyClinic>>>; saving: boolean; editing: boolean; onClose: () => void; onSave: () => void }) {
  const colors = useAppColors();
  const enhancedStyles = React.useMemo(() => createEnhancedStyles(colors), [colors]);
  const styles = React.useMemo(() => createStyles(colors), [colors]);
 const latitude = Number(form.latitud); const longitude = Number(form.longitud); const hasCoordinates = Boolean(form.latitud && form.longitud && !Number.isNaN(latitude) && !Number.isNaN(longitude)); return <FormModal visible={visible} title={editing ? 'Editar clínica' : 'Nueva clínica'} saving={saving} onClose={onClose} onSave={onSave}><Field label="Nombre *" value={form.nombre} onChangeText={(nombre) => setForm((old) => ({ ...old, nombre }))}/><AppText style={styles.fieldLabel}>Tipo</AppText><View style={styles.pills}>{['clinica', 'hospital', 'laboratorio'].map((tipo) => <TouchableOpacity key={tipo} style={[styles.pill, form.tipo === tipo && styles.pillActive]} onPress={() => setForm((old) => ({ ...old, tipo }))}><AppText style={[styles.pillText, form.tipo === tipo && styles.pillTextActive]}>{tipo}</AppText></TouchableOpacity>)}</View><Field label="Ciudad" value={form.ciudad} onChangeText={(ciudad) => setForm((old) => ({ ...old, ciudad }))}/><Field label="Departamento" value={form.departamento} onChangeText={(departamento) => setForm((old) => ({ ...old, departamento }))}/><Field label="Dirección" value={form.direccion} onChangeText={(direccion) => setForm((old) => ({ ...old, direccion }))} multiline/><View style={enhancedStyles.locationTitle}><Ionicons name="location-outline" size={17} color={colors.info}/><AppText style={styles.fieldLabel}>Ubicación en el mapa</AppText></View><LocationMapPicker latitude={hasCoordinates ? latitude : null} longitude={hasCoordinates ? longitude : null} onLocationChange={(latitud: number, longitud: number) => setForm((old) => ({ ...old, latitud: latitud.toFixed(6), longitud: longitud.toFixed(6) }))}/><View style={enhancedStyles.coordinateRow}><View style={enhancedStyles.coordinateField}><Field label="Latitud" value={form.latitud} onChangeText={(latitud) => setForm((old) => ({ ...old, latitud }))} keyboardType="default"/></View><View style={enhancedStyles.coordinateField}><Field label="Longitud" value={form.longitud} onChangeText={(longitud) => setForm((old) => ({ ...old, longitud }))} keyboardType="default"/></View></View><Field label="Teléfono" value={form.telefono} onChangeText={(telefono) => setForm((old) => ({ ...old, telefono }))} keyboardType="phone-pad"/><Field label="Correo" value={form.correo} onChangeText={(correo) => setForm((old) => ({ ...old, correo }))} keyboardType="email-address"/><Field label="Horario de atención" value={form.horarioAtencion} onChangeText={(horarioAtencion) => setForm((old) => ({ ...old, horarioAtencion }))}/><BooleanRow label="Institución activa" value={form.activo} onChange={(activo) => setForm((old) => ({ ...old, activo }))}/></FormModal>; }
...
const createStyles = (colors: AppColors) => StyleSheet.create({ safe: { flex: 1, backgroundColor: colors.background }, container: { padding: 18, gap: 12 }, access: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: 14, padding: 30, backgroundColor: colors.background }, hero: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, padding: 18, backgroundColor: colors.surfaceStrong, borderColor: colors.borderStrong, borderWidth: 1, borderRadius: 20 }, heroIcon: { width: 48, height: 48, borderRadius: 15, alignItems: 'center', justifyContent: 'center', backgroundColor: `${colors.info}1A` }, heroCopy: { flex: 1 }, eyebrow: { fontSize: 11, color: colors.info, fontWeight: '800', letterSpacing: 1 }, title: { color: colors.text, fontSize: 23, fontWeight: '900', marginTop: 2 }, subtitle: { color: colors.textSoft, fontSize: 13, lineHeight: 19, marginTop: 4 }, refresh: { padding: 8 }, tabs: { flexDirection: 'row', gap: 8 }, tab: { flex: 1, minHeight: 44, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 7, borderRadius: 12, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface }, tabActive: { backgroundColor: colors.info, borderColor: colors.info }, tabText: { color: colors.textMuted, fontWeight: '800' }, tabTextActive: { color: colors.onAccent }, primary: { minHeight: 48, borderRadius: 13, backgroundColor: colors.success, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, paddingHorizontal: 15 }, primaryText: { color: colors.onAccent, fontWeight: '900' }, card: { padding: 15, gap: 11, borderRadius: 16, backgroundColor: colors.surface, borderWidth: 1, borderColor: colors.border }, cardTop: { flexDirection: 'row', alignItems: 'center', gap: 10 }, cardIcon: { width: 40, height: 40, alignItems: 'center', justifyContent: 'center', borderRadius: 12, backgroundColor: `${colors.info}18` }, cardCopy: { flex: 1 }, cardTitle: { color: colors.text, fontSize: 16, fontWeight: '900' }, cardMeta: { color: colors.textMuted, fontSize: 12, marginTop: 2, textTransform: 'capitalize' }, detail: { color: colors.textSoft, fontSize: 13 }, status: { paddingHorizontal: 7, paddingVertical: 4, borderRadius: 99 }, actions: { flexDirection: 'row', gap: 9 }, smallAction: { flexDirection: 'row', alignItems: 'center', gap: 5, paddingVertical: 7, paddingHorizontal: 9, borderRadius: 9, backgroundColor: `${colors.info}14` }, smallText: { color: colors.info, fontSize: 12, fontWeight: '800' }, helper: { color: colors.textMuted, textAlign: 'center', lineHeight: 19, padding: 10 }, empty: { padding: 32, alignItems: 'center', gap: 10, borderWidth: 1, borderStyle: 'dashed', borderColor: colors.border, borderRadius: 16 }, selection: { backgroundColor: `${colors.info}14`, borderColor: `${colors.info}50`, borderWidth: 1, padding: 13, borderRadius: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, selectionLabel: { color: colors.info, fontSize: 10, fontWeight: '900', letterSpacing: 1 }, selectionName: { color: colors.text, fontWeight: '900', marginTop: 2 }, loader: { marginVertical: 36 }, error: { padding: 12, borderRadius: 12, backgroundColor: `${colors.accent}18`, borderWidth: 1, borderColor: `${colors.accent}50`, gap: 7 }, errorText: { color: colors.textSoft }, retry: { color: colors.info, fontWeight: '800' }, overlay: { flex: 1, justifyContent: 'flex-end', backgroundColor: '#000000A8' }, modal: { maxHeight: '92%', padding: 20, backgroundColor: colors.surfaceStrong, borderTopLeftRadius: 25, borderTopRightRadius: 25, gap: 12 }, modalHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }, modalTitle: { color: colors.text, fontSize: 20, fontWeight: '900' }, field: { gap: 6, marginBottom: 12 }, fieldLabel: { color: colors.textSoft, fontSize: 12, fontWeight: '800', marginBottom: 6 }, input: { minHeight: 46, borderRadius: 11, paddingHorizontal: 13, color: colors.text, backgroundColor: colors.backgroundMuted, borderWidth: 1, borderColor: colors.border }, textarea: { minHeight: 78, paddingTop: 12, textAlignVertical: 'top' }, pills: { flexDirection: 'row', gap: 7, marginBottom: 14 }, pill: { borderWidth: 1, borderColor: colors.border, borderRadius: 99, paddingHorizontal: 11, paddingVertical: 7 }, pillActive: { backgroundColor: colors.info, borderColor: colors.info }, pillText: { color: colors.textMuted, fontSize: 12, textTransform: 'capitalize' }, pillTextActive: { color: colors.onAccent, fontWeight: '900' }, booleanRow: { flexDirection: 'row', alignItems: 'center', gap: 9, paddingVertical: 9 }, checkbox: { height: 21, width: 21, borderWidth: 1, borderColor: colors.border, borderRadius: 6, alignItems: 'center', justifyContent: 'center' }, checkboxActive: { backgroundColor: colors.success, borderColor: colors.success }, booleanText: { color: colors.textSoft, fontSize: 13, fontWeight: '700' }, disabled: { opacity: .6 } });
