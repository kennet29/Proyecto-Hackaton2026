...
export function NanoChefScreen({ navigation }: Props) {
  ...
  const recipePages = useMemo(() => {
    ...
    return [
      ...
      { key: 'tip', title: 'Consejo de Nano', icon: 'sparkles-outline' as const, content: <View style={styles.tipBox}><Ionicons name="sparkles" size={23} color={colors.success} /><AppText style={styles.tipText}>{recipe.nanoTip}</AppText></View> },
    ];
  }, [recipe, selectedGoal.label, colors, styles]);
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView contentContainerStyle={[styles.container, isWebWide && styles.containerWeb]} showsVerticalScrollIndicator={false}>
            ...
            {RECIPE_GOALS.map((goal) => {
              ...
              return (
                <TouchableOpacity key={goal.id} style={[styles.goalCard, selected && { borderColor: goal.accent, backgroundColor: colorAlpha(goal.accent, '18') }]} onPress={() => setGoalId(goal.id)}>
                  ...
                  <Ionicons name={selected ? 'radio-button-on' : 'radio-button-off'} size={18} color={selected ? goal.accent : colors.textMuted} />
                </TouchableOpacity>
              );
            })}
          </View>
        ...
        <TouchableOpacity style={[styles.primaryButton, submitting && styles.buttonDisabled]} onPress={() => void createRecipe()} disabled={submitting}>
          {submitting ? <ActivityIndicator color={colors.onAccent} /> : <Ionicons name="restaurant-outline" size={19} color={colors.onAccent} />}
...
const createStyles = (colors: AppColors) => StyleSheet.create({
  ...
  infoCard: { flexDirection: 'row', gap: 10, borderRadius: 16, padding: 15, backgroundColor: colorAlpha(colors.success, '12'), borderWidth: 1, borderColor: colorAlpha(colors.success, '55') },
  ...
  section: { gap: 9 }, sectionTitle: { color: colors.text, fontSize: 16, fontWeight: '900' }, sectionSubtitle: { color: colors.textMuted, fontSize: 13 },
  ...
  recommendationNotice: { flexDirection: 'row', gap: 9, borderRadius: 13, padding: 12, backgroundColor: colorAlpha(colors.success, '12') }, recommendationNoticeText: { flex: 1, color: colors.textSoft, fontSize: 13, lineHeight: 19 },
  chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 7 }, chip: { borderWidth: 1, borderColor: colorAlpha(colors.info, '66'), backgroundColor: colorAlpha(colors.info, '12'), borderRadius: 99, paddingVertical: 7, paddingHorizontal: 10 }, chipText: { color: colors.info, fontSize: 12, fontWeight: '800' },
  primaryButton: { minHeight: 52, borderRadius: 14, backgroundColor: colors.success, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 9 }, primaryButtonText: { color: colors.onAccent, fontSize: 15, fontWeight: '900' }, buttonDisabled: { opacity: 0.65 },
  recipeCard: { borderRadius: 18, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, padding: 16, gap: 12 }, recipeHeader: { flexDirection: 'row', alignItems: 'center', gap: 8 }, recipeTitle: { color: colors.text, fontSize: 17, fontWeight: '900' },
  carouselProgress: { flexDirection: 'row', gap: 6 }, progressDot: { flex: 1, height: 4, borderRadius: 4, backgroundColor: colors.border }, carouselPage: { minHeight: 215, gap: 14 }, carouselTitleRow: { flexDirection: 'row', alignItems: 'center', gap: 10 }, carouselIcon: { width: 38, height: 38, borderRadius: 12, alignItems: 'center', justifyContent: 'center' }, carouselTitle: { flex: 1, color: colors.text, fontSize: 18, fontWeight: '900' }, carouselDescription: { color: colors.textSoft, fontSize: 14, lineHeight: 21 }, recipeMeta: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, metaPill: { flexDirection: 'row', alignItems: 'center', gap: 6, borderRadius: 99, paddingHorizontal: 10, paddingVertical: 7, backgroundColor: colorAlpha(colors.info, '12') }, metaText: { color: colors.textSoft, fontSize: 12, fontWeight: '800' }, list: { gap: 10 }, listRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 10 }, listBullet: { minWidth: 24, height: 24, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: colorAlpha(colors.info, '22') }, stepBullet: { backgroundColor: colorAlpha(colors.success, '22') }, listBulletText: { color: colors.text, fontSize: 11, fontWeight: '900' }, listText: { flex: 1, color: colors.textSoft, fontSize: 14, lineHeight: 21 }, tipBox: { flexDirection: 'row', alignItems: 'flex-start', gap: 11, padding: 14, borderRadius: 14, backgroundColor: colorAlpha(colors.success, '12'), borderWidth: 1, borderColor: colorAlpha(colors.success, '55') }, tipText: { flex: 1, color: colors.textSoft, fontSize: 14, lineHeight: 21 }, carouselNav: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 8 }, carouselButton: { flexDirection: 'row', alignItems: 'center', gap: 2, paddingVertical: 8 }, carouselButtonDisabled: { opacity: 0.35 }, carouselButtonText: { color: colors.info, fontSize: 13, fontWeight: '800' }, pageLabel: { color: colors.textMuted, fontSize: 12, fontWeight: '800' },
  newRecipeButton: { alignSelf: 'flex-start', flexDirection: 'row', alignItems: 'center', gap: 7, paddingVertical: 8 }, newRecipeText: { color: colors.info, fontWeight: '800', fontSize: 13 },
  recipePlaceholder: { minHeight: 250, padding: 24, borderRadius: 18, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface, alignItems: 'center', justifyContent: 'center', gap: 10 },
  recipePlaceholderIcon: { width: 62, height: 62, borderRadius: 22, alignItems: 'center', justifyContent: 'center', backgroundColor: colorAlpha(colors.success, '18') }, recipePlaceholderTitle: { color: colors.text, fontSize: 17, fontWeight: '900', textAlign: 'center' }, recipePlaceholderText: { color: colors.textMuted, fontSize: 13, lineHeight: 20, textAlign: 'center', maxWidth: 310 },
});
